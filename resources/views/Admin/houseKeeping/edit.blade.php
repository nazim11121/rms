                          ...
                          <form action="{{route('house-keeping.update', $data->id)}}" method="POST">
                              ...
                              <div class="row">
                                  <div class="col-md-12 mb-3">
                                      ...
                                      <select id="housekeeping-item" name="amenities_id[]" class="form-select select2" multiple required>
                                          ...
                                          @foreach($amenitiesList as $amenity)
                                              <option value="{{$amenity->id}}" {{ in_array($amenity->id, $selectedItems) ? 'selected' : '' }}>{{$amenity->name}}</option>
                                          @endforeach
                                      </select>
                                  </div>
                                  <div class="col-md-12 mb-3">
                                      ...
                                      <select class="form-select" name="room_no" required>
                                          ...
                                          @foreach($roomList as $room)
                                              <option value="{{$room->id}}" {{$data->room_no==$room->id?'selected':''}}>{{$room->room_no}}</option>
                                          @endforeach
                                      </select>
                                  </div>
                                  <div class="col-md-6 mb-3">
                                      ...
                                      <div class="d-flex">
                                          <select class="form-select" name="status" required>
                                              <option value="">--Select--</option>
                                              <option value="1" {{$data->status==1?'selected':''}}>Clean</option>
                                              <option value="0" {{$data->status==0?'selected':''}}>Dirty</option>
                                          </select>
                                      </div>
                                  </div>
                                  <div class="col-md-6 mb-3">
                                  ...
                                  <div class="col-md-12 mb-3">
                                      ...
                                      <textarea class="form-control" name="description" rows="3" placeholder="Enter remark" value="{{$data->description}}">{{$data->description}}</textarea>
                                  </div>
                              </div>
